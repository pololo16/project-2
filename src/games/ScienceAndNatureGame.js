...
import { getScienceAndNatureGame } from '../lib/api'
import { useHistory } from 'react-router-dom'



function ScienceAndNatureGame() {
  ...
  React.useEffect(() => {
    const getData = async () => {
      const response = await getScienceAndNatureGame() 
      ...
      <section className="hero is-small-with-navbar is-info">
        <div className="hero-body">
          <div className="container">
            <h1 className="title is-1 has-text-centered">SCIENCE and NATURE</h1>
          </div>
        </div>
      </section>
...
export default ScienceAndNatureGame